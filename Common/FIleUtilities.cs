...
namespace AxialManagerS_Converter.Common {
  ...
  public class FIleUtilities {
    ...
    /// 特定の文字列を含むファイル名を探索する
    /// </summary>
    /// <param name="dirName">探索するディレクトリ</param>
    /// <param name="searchString">検索する文字列</param>
    /// <param name="files">結果リスト</param>
    public void SearchFilesContainingString(string dirName, string searchString, ref List<string> files) {
      if (string.IsNullOrEmpty(dirName) || string.IsNullOrEmpty(searchString)) return;

      try {
        // 指定されたディレクトリ内のすべてのファイルを取得
        string[] allFiles = Directory.GetFiles(dirName, "*", SearchOption.AllDirectories);

        // 特定の文字列を含むファイル名をフィルタリング
    ...
    /// <returns></returns>
    public bool SelectFolder(ref string dirName, string msg, string currentPath, string delimiter) {
      bool apply = false;
      ...
      if (dialog.ShowDialog() == DialogResult.OK) {
        if (dialog.SelectedPath.Length > 0) {
          dirName = dialog.SelectedPath + delimiter;
          apply = true;
        }
      }
    ...
    /// <param name="delimiter"></param>
    /// <returns></returns>
    public bool SelectFile(ref string fileName, string currentPath, string currentFileName, string filter, string delimiter) {
      bool apply = false;
