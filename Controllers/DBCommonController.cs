...
namespace AxialManagerS_Converter.Controllers {
  ...
  public class DBCommonController {
    ...
    public static int RegisterExamList(string pt_uuid, string dataType, DBConst.eEyeType posEye, DateTime? exam_datetime, int deviceId, NpgsqlConnection sqlConnection) {
      int retExamId = -1;
      ...
      try {
        ...
        examrec.eye_id = Select_Eye_ID(sqlConnection, DBConst.strEyeType[posEye]);
    ...
    public static string _doupdateintlist(string col, List<int?> list) {
      string valueList = "{";
      valueList += string.Join(",", list.Select((x) => x.HasValue ? x.ToString() : "0"));    // todo: 0は適切か
      valueList += "}";
